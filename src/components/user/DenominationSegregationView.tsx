      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        {/* LEFT: Denomination Calculator */}
              ...
              {crossOutletId && (
                <>
                  <div className={`flex items-center gap-1 p-1 rounded-lg ${isDark ? 'bg-[#0F172A]' : 'bg-white'}`}>
                    ...
                    >
                                          {activeType === 'CW' || activeType === 'LD' ? `Cash Paid From ${crossOutlet?.name}` : `Cash Received at ${crossOutlet?.name}`}
                    </button>
                  </div>
          ...
          {selectedAccount && (
            ...
            disabled={
              isChangeMode
                ? changeReceivedTotal <= 0 || changeReceivedTotal !== totalReceivedAmount
                : !selectedAccount || totalReceivedAmount <= 0
            }
