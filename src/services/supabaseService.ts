...
export const mapDbToSegregation = (row: any): DenominationSegregationRecord => {
  return {
    ...
    customerName: row.customer_name || '',
    mobileNumber: row.mobile_number || '',
    bearerName: row.bearer_name || undefined,
    outletId: row.outlet_id || '',
    outletName: row.outlet_name || '',
       userId: row.user_id,
    userName: row.user_name,
...
export const mapSegregationToDb = (record: DenominationSegregationRecord) => {
  return {
    ...
    change_note_1: record.changeReceivedDenominations?.note1 || 0,
    change_note_2: record.changeReceivedDenominations?.note2 || 0,
    change_note_5: record.changeReceivedDenominations?.note5 || 0,
    change_note_10: record.changeReceivedDenominations?.note10 || 0,
    change_note_20: record.changeReceivedDenominations?.note20 || 0,
    change_note_50: record.changeReceivedDenominations?.note50 || 0,
    change_note_100: record.changeReceivedDenominations?.note100 || 0,
    change_note_200: record.changeReceivedDenominations?.note200 || 0,
    change_note_500: record.changeReceivedDenominations?.note500 || 0,
    change_note_1000: record.changeReceivedDenominations?.note1000 || 0,
    ...
    customer_name: record.customerName,
    mobile_number: record.mobileNumber,
    bearer_name: record.bearerName || null,
       outlet_id: record.outletId,
    outlet_name: record.outletName,
    user_id: record.userId,
    user_name: record.userName,
...
export const mapDbToSupporting = (row: any): SupportingRecord => {
  return {
    ...
    recoveredFundingSource: row.recovered_funding_source || undefined,
    recoveredAmount: row.recovered_amount ? Number(row.recovered_amount) : undefined,
    recoveredDenominations: row.recovered_funding_source === 'CASH' ? {
      note1: Number(row.recovered_note_1 || 0),
      note2: Number(row.recovered_note_2 || 0),
      note5: Number(row.recovered_note_5 || 0),
      note10: Number(row.recovered_note_10 || 0),
      note20: Number(row.recovered_note_20 || 0),
      note50: Number(row.recovered_note_50 || 0),
      note100: Number(row.recovered_note_100 || 0),
      note200: Number(row.recovered_note_200 || 0),
      note500: Number(row.recovered_note_500 || 0),
      note1000: Number(row.recovered_note_1000 || 0)
    } : undefined,
    outletId: row.outlet_id || '',
    outletName: row.outlet_name || '',
    userId: row.user_id,
...
export const mapSupportingToDb = (record: SupportingRecord) => {
  return {
    ...
    recovered_funding_source: record.recoveredFundingSource || null,
    recovered_amount: record.recoveredAmount || 0,
    recovered_note_1: record.recoveredDenominations?.note1 || 0,
    recovered_note_2: record.recoveredDenominations?.note2 || 0,
    recovered_note_5: record.recoveredDenominations?.note5 || 0,
    recovered_note_10: record.recoveredDenominations?.note10 || 0,
    recovered_note_20: record.recoveredDenominations?.note20 || 0,
    recovered_note_50: record.recoveredDenominations?.note50 || 0,
    recovered_note_100: record.recoveredDenominations?.note100 || 0,
    recovered_note_200: record.recoveredDenominations?.note200 || 0,
    recovered_note_500: record.recoveredDenominations?.note500 || 0,
    recovered_note_1000: record.recoveredDenominations?.note1000 || 0,
    outlet_id: record.outletId,
    outlet_name: record.outletName,
    user_id: record.userId,
    user_name: record.userName,
...
export const SupabaseService = {
  ...
  // Verify user password server-side via RPC (password never leaves the DB)
  async verifyUserPassword(identifier: string, password: string): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        console.warn('Error verifying password via RPC:', error.message);
        return false;
      }
      return !!data;
    } catch (err) {
      console.warn('Unexpected error verifying password:', err);
      return false;
    }
  },
  ...
  async fetchAllData() {
    ...
    try {
        ...
        supabase.from('all_accounts').select('*').order('created_at', { ascending: false }),
        supabase.from('denomination_segregations').select('*').order('created_at', { ascending: false }),
        supabase.from('supporting_records').select('*').order('created_at', { ascending: false }),
        ...
        supabase.from('afo_notes').select('*').order('created_at', { ascending: false }),
        supabase.from('outlet_rules').select('*').order('created_at', { ascending: false }),
      ...
      return {
        ...
        allAccounts: (allAccountsRes.data || []).map(mapDbToAllAccount),
        segregationRecords: (segregationsRes.data || []).map(mapDbToSegregation),
        supportingRecords: (supportingRes.data || []).map(mapDbToSupporting),
        ...
        notes: (notesRes.data || []).map(mapDbToNote),
        rules: (rulesRes.data || []).map(mapDbToRule),
