...
export interface AppContextType {
  ...
  transferAfoToOutlet: (
    ...
    options?: { revokePreviousOutletAccess?: boolean; adminNote?: string }
     ...
     delegateAdminAccess: (data: { email: string; fullName: string; username: string; pin: string; permissions: AdminPermission[]; assignedOutletIds?: string[] }) => Promise<{ success: boolean; message?: string }>;
      ...
      if (SupabaseService.isAvailable()) {
        try {
          ...
          if (cloudData && isMounted) {
            ...
            if (cloudData.allAccounts) setAllAccounts(cloudData.allAccounts);
            if (cloudData.segregationRecords) setSegregationRecords(cloudData.segregationRecords);
            if (cloudData.motherAmounts) setMotherAmounts(cloudData.motherAmounts);
            if (cloudData.supportingRecords) setSupportingRecords(cloudData.supportingRecords);
            if (cloudData.outletTransfers && cloudData.outletTransfers.length) setOutletTransfers(cloudData.outletTransfers);
...
if (sessionStatus.isActive) {
  ...
  // Denomination Segregation Module — Save Segregation Transaction
    ...
    const newRecord: DenominationSegregationRecord = {
      ...
      changeReceivedDenominations: data.changeReceivedDenominations,
      outletId: user.outletId || 'OUT-DHK-001',
      outletName: user.outletName || 'Motijheel Commercial SME Outlet',
      userId: user.id,
      userName: user.fullName,
  ...
  const deleteRule = (id: string) => {
    ...
    const destinationOutlet = data.destinationOutletId ? outlets.find((o) => o.id === data.destinationOutletId) : undefined;
      ...
      destinationOutletName: destinationOutlet?.name,
      userId: user.id,
      userName: user.fullName,
      note: data.note,
      createdAt: new Date().toISOString()
    ...
    // If a charge is applied, only the amount AFTER deducting the charge is added to Mother Amount
    if (data.transferType === 'RTGS') {
      const rtgsChargeAmt = data.chargeAmount || 0;
      const rtgsNetAmount = data.amount - rtgsChargeAmt;
      ...
      addMotherAmount({
        outletId: data.outletId,
        amount: currentMotherAmountRtgs + rtgsNetAmount,
        note: rtgsChargeAmt > 0
          ? `RTGS transfer of ৳${data.amount} (Charge: ৳${rtgsChargeAmt}, Net: ৳${rtgsNetAmount})`
          : `RTGS transfer of ৳${data.amount}`
      });
    }

    addAuditEntry(
      'CASH_TRANSFER_ADDED',
      `${data.transferType} transfer of ৳${data.amount} added for ${outlet?.name}${destinationOutlet ? ` → ${destinationOutlet.name}` : ''}`,
      outlet?.name
    );
    showToast({
      message: `${
